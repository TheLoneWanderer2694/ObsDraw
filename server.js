...
// 3. REAL-TIME SOCKET.IO BROADCAST LOGIC
io.on('connection', (socket) => {
  console.log('Client connected:', socket.id);

  // Use io.emit so both viewers and the OBS overlay update simultaneously
  socket.on('draw', (data) => {
    io.emit('draw', data);
  });
...
server.listen(PORT, () => console.log(`Server running on port ${PORT}`));    const lineWidth = document.getElementById('lineWidth');
    const clearBtn = document.getElementById('clearBtn');
